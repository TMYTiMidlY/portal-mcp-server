...
|-----------------------|--------------------------------|------------------------------------------------------------------------------|
| Prompt-layer rules    | Agent system prompt / `AGENTS.md` | The agent is expected to default writes to remote `/tmp/`, ask before touching `$HOME` or project source, and never mix portal's tool calls with raw `ssh`/`scp` in the same task. See the README's *Agent-side conventions* section. |
...
- **Semantic safety net (opt-in)** — when `safety_net.enabled: true`, each
  ...
  agent's own `bash` tool and so never sees commands issued through portal's
...
- **Socket hardening** — the user `.socket` unit listens on
  ...
  `PORTAL_CREDENTIAL_AGENT_SOCKET`) instead of guessing a runtime dir. On the
  Linux/macOS AF_UNIX transport the agent calls
  `getsockopt(SO_PEERCRED)` (Linux) / `LOCAL_PEERCRED` on every accepted connection (and the
  ...
  agent refuses to cache anything from a foreign uid. Auto-install covers
  Linux (systemd user units), macOS (launchd LaunchAgent) and Windows
  (per-user logon scheduled task + named pipe); on Windows the same-user
  boundary is enforced by a named-pipe peer-SID check that **fails closed**.
  Each backend supervises a single per-user credential agent service.
...
- **Socket hardening** — the user `.socket` unit listens on
  ...
  `agent.json`, and clients use that config (or an explicit
  `PORTAL_CREDENTIAL_AGENT_SOCKET`) instead of guessing a runtime dir. On the
  Linux/macOS AF_UNIX transport the agent also calls
  `getsockopt(SO_PEERCRED)` (Linux) / `LOCAL_PEERCRED` on every accepted connection (and the
  client mirrors it after `connect`) and closes the socket on a uid
  ...
  refuses to cache anything from a foreign uid. Auto-install covers Linux
  (systemd), macOS (launchd) and Windows (per-user scheduled task + named
  pipe); on Windows the same-user boundary is a named-pipe peer-SID check that
  **fails closed**. Each backend supervises a single per-user credential agent
  service, so a second MCP process cannot hijack the channel.
...
- Password-based SSH authentication is opt-in via `password_command:` in
  `hosts.yaml` (an external shell command that prints the password to stdout)
  **or** the out-of-band `portal ssh set <host>` credential-agent side-channel;
  plaintext `password:` fields and any MCP tool parameter for passwords are
