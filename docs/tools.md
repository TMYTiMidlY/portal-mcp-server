...
| `portal_local_exec` | `(command, secrets=None, use_sudo=False, timeout=3600.0)` | Run a command on the **MCP server's own machine** (not over SSH). OFF by default — an off-target derivative of the project's remote-driving goal — unless the operator explicitly sets `PORTAL_ALLOW_LOCAL_EXEC=1`. For tasks that genuinely belong on the server host (e.g. a local secret); remote work goes through `portal_exec`. `use_sudo=True` runs it under local `sudo -S -k` using the reserved **`<local>`** identity (password from `portal sudo set-local` or a top-level `<local>:` section's `sudo_password_command` in hosts.yaml — never a parameter); may be combined with `secrets` (values ride stdin inside the elevated shell, bypassing sudo's env_reset), flags `high_risk`, audits as `local_exec_sudo`. NB `<local>` (this machine) ≠ `local` / `localhost` (ordinary SSH hosts). |
...
> **timeout (per command, seconds).** `portal_exec` / `portal_shell` /
> `portal_local_exec` all default to 1h — but that's just the default for when
> the agent omits `timeout`; set `PORTAL_DEFAULT_TIMEOUT` (seconds) in the
> server's `env` to lower it globally. Because the keepalive heartbeats keep the
> client from aborting a hung call on its own, `timeout` is effectively the only
> real cut-off, so for an exploratory / safely-interruptible command the agent
> should pass a SMALL `timeout` first and raise it only once it knows the
> command is genuinely slow. A per-call `timeout` always overrides
> `PORTAL_DEFAULT_TIMEOUT`.
...
| Tool | Signature | Purpose |
|---|---|---|
| `portal_read` | `(host, path, start=1, end=None, limit=None, encoding="utf-8")` | Read a file (or 1-based line range) and return `{content, file_hash, range_hash, start, end, total_lines, truncated}`. The two SHA-256 hashes are required by `portal_patch`. **Paged**: one call returns at most `limit` lines (default `PORTAL_READ_MAX_LINES=2000`) and at most `PORTAL_READ_MAX_BYTES` (default 16384) of content — so a big file never comes back as one oversized blob the MCP client has to spill to a temp file. When the page stops before the requested end, `truncated` is `true` and `next_start` gives the line to continue from. Pages cut on line boundaries, so `range_hash` stays valid for a follow-up patch. |
