...
| `portal_local_exec` | `(command, secrets=None, timeout=3600.0)` | Run a command on the **MCP server's own machine** (not over SSH). DISABLED unless the operator sets `PORTAL_ALLOW_LOCAL_EXEC=1` (larger threat surface). For tasks that genuinely belong on the server host (e.g. a local secret); remote work goes through `portal_exec`. No sudo (sudo is remote-only via `portal_exec(use_sudo=True)`). |
