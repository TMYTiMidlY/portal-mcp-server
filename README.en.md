...
Resolution order mirrors the remote path: **agent cache → the top-level `<local>:` `sudo_password_command` → error**. `use_sudo` and `secrets` **may be combined** in one `portal_local_exec` call (the secret values ride stdin after the sudo password and are read+exported inside the elevated shell, so sudo's `env_reset` can't strip them and nothing lands on argv); the result is flagged `high_risk` and audited as `local_exec_sudo`.
...
See [`examples/secrets.yaml`](./examples/secrets.yaml). `secrets` and `use_sudo` may be combined in a single `portal_exec` call — the secret values ride stdin after the sudo password and are read+exported inside the elevated shell, bypassing sudo's `env_reset` with no sudoers `env_keep` config and without touching argv.
