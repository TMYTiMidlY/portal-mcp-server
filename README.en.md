...
### Credential agent (Linux systemd / macOS launchd)

> **⚠️ Auto-install platform: Linux + macOS** (Windows transport works via a manual run — see below). `portal agent install` dispatches by OS: **Linux** ships a pair of **systemd user units** (`.socket` + `.service` under `~/.config/systemd/user/`, lazily started via socket activation); **macOS** ships a **launchd LaunchAgent** (`~/Library/LaunchAgents/com.tmytimidly.portal-credential-agent.plist`, run-and-keepalive — the agent self-binds its AF_UNIX socket, sidestepping the `launch_activate_socket` ctypes dance). **Windows**: the agent's IPC transport is a **named pipe** (no AF_UNIX) and it works — it's exercised end-to-end by the `windows-latest` CI job — so you can run it manually with `portal agent run --socket \\.\pipe\portal-mcp-server-credentials-<user>` (set the same value in `PORTAL_CREDENTIAL_AGENT_SOCKET` for the MCP server). Only the auto-start-at-logon **service** install is not wired up yet, so `portal agent install` still prints an actionable hint there.
>
> Where there's no agent at all, the alternative: use the `password_command` / `passphrase_command` / `sudo_password_command` fields in `hosts.yaml`, or the `command:` field in `secrets.yaml`, to pull credentials on demand from the system password manager (Keychain, `pass`, `secret-tool`, `gopass`, 1Password CLI, etc.) — see [Authentication](#authentication) below. The MCP server itself (`portal_shell` and every remote tool) runs fine on Windows / macOS / Linux; only this **local interactive no-echo password-staging** path depends on an OS service manager (or a manual `portal agent run`).
