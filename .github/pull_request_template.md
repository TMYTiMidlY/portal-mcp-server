## What

<!-- What does this PR do? One-sentence summary. -->

## Why

<!-- Link to issue, user scenario, or upstream advisory. -->

## How

<!-- Key design decisions, if any. Delete this section for trivial changes. -->

## Test plan

<!-- What did you run? Paste relevant output. -->

- [ ] `pytest tests/ -v` is all green
- [ ] `ruff check portal_mcp_server/` passes
- [ ] If adding/changing a tool: updated `docs/tools.md`
- [ ] If adding a config field: updated `config/hosts.example.yaml`
- [ ] No secrets, hardcoded IPs, or local paths in the diff
